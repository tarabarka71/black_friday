...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard_id, shard_host in shards.items():
            ...
            host = rs_info[1].split(",")[0]
            rs_name = rs_info[0]
            motor_client = motor.motor_asyncio.AsyncIOMotorClient(f"mongodb://{host}")
            client_db = motor_client[DATABASE_NAME]
            shard_count = await client_db.get_collection("helloDoc").count_documents({})
            docs_in_shard[shard_id] = shard_count
                
            replSetGetStatus = await motor_client.admin.command("replSetGetStatus")
            replica_info = {
                "replica_set_name": replSetGetStatus.get("set"),
                "members": []
            }
                    
            for member in replSetGetStatus.get("members", []):
                member_info = {
